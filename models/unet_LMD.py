...
class UNet_LMD(nn.Module):
    def __init__(self, activefunc,droprate,kernel_size,n_channels, n_classes, bilinear=True):
        ...
        self.up4 = LMD.CR_B(in_channel=128,out_channel=cfg.y0_channel,kernel_size=self.ker,stride=1,padding=self.pad)
        self.up5 = LMD.CR_B(in_channel=64, out_channel=cfg.y0_channel,kernel_size=self.ker,stride=1,padding=self.pad)
...
if __name__ == '__main__':
    net = UNet_LMD(activefunc='relu',droprate=0.1,kernel_size=3,n_channels=3, n_classes=1).cuda()
